...
export const useAvailableWorkers = (
  params?: Omit<GetWorkersRequest, "availability">,
) => {
  const {
  ...
  const availableWorkers = useMemo(
    () => availableWorkersData?.data || [],
    [availableWorkersData?.data],
  );
  const pagination = useMemo(
    () => availableWorkersData?.pagination,
    [availableWorkersData?.pagination],
  );
...
export const useBusyWorkers = (
  ...
  const busyWorkers = useMemo(
    () => busyWorkersData?.data || [],
    [busyWorkersData?.data],
  );
  const pagination = useMemo(
    () => busyWorkersData?.pagination,
    [busyWorkersData?.pagination],
  );
...
export const useWorker = (workerId: string) => {
  const {
    ...
    isFetching,
    refetch,
  } = useGetWorkerByIdQuery({ id: workerId }, { skip: !workerId });
...
export const useWorkerStatusUpdate = () => {
  const [updateWorkerStatus, { isLoading, error }] =
    useUpdateWorkerStatusMutation();
...
export const useWorkerProfileUpdate = () => {
  const [updateWorkerProfile, { isLoading, error }] =
    useUpdateWorkerProfileMutation();

  const updateProfile = async (
    workerId: string,
    profileData: Parameters<typeof updateWorkerProfile>[0]["data"],
  ) => {
    try {
...
export const useWorkerSearch = (
  searchParams: GetWorkersRequest & { query?: string },
) => {
  const {
    data: searchResults,
    error,
    ...
    skip:
      ...
      !Object.keys(searchParams).some(
        (key) =>
          key !== "query" && searchParams[key as keyof typeof searchParams],
      ),
  });
...
export const useTopRatedWorkers = (params?: {
  limit?: number;
...
export const useWorkersData = () => {
  ...
  const { updateStatus, isLoading: statusUpdateLoading } =
